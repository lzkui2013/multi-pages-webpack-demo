...
async function getDep (buildType) {
    try {
        ...
        let {commitHash, diffArr, commitMsg} = await getGitDiff(repo, beforeCommitHash, {branch: 'master'});
        outPutTime('比对文件差异');
        ...
        // // 去执行
        // await doShellAction(shellAction, rootPath, outPutPath);

        // // 提交线上代码仓库
        // await doGitCommit(outPutPath, commitMsg);
